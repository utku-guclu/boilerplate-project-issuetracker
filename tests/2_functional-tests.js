...
suite("Functional Tests", function () {
  ...
  suite("PUT /api/issues/{project} => text", function () {
    test("No _id in request", function (done) {
      chai
        .request(server)
        .put("/api/issues/test")
        .send({
    ...
    test("No update fields in request", function (done) {
      chai
        .request(server)
        .put("/api/issues/test")
        .send({
    ...
    test("Invalid _id in request", function (done) {
      chai
        .request(server)
        .put("/api/issues/test")
        .send({
        ...
        .end(function (err, res) {
          assert.deepEqual(res.body, { error: 'Error updating issue', '_id': 'invalid_id' });
          done();
        });
    });
